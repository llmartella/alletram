#!/usr/bin/env python3
"""
generate_sql_credit.py
----------------------
Reads the 'info' tab from a Google Sheet mapping doc and generates one DuckDB
SQL file per Excel file referenced in the mapping, targeting the
transaction_mapping_credit table.

Schema differences from transaction_mapping_base:
  - No contractor_number / contractor_name
  - Adds material_group_number (text)

Usage:
    python generate_sql_credit.py   (uses hardcoded defaults below)

Requires credentials.json and token.pkl in the same directory as this script.
"""
...
FIELD_SCHEMA: dict[str, tuple[str, str | None]] = {
    ...
    "item_upc":                 ("text",            None),
    "material_group_number":    ("text",            None),
...
# Always static literal fields (never column refs)
ALWAYS_STATIC = {
    "wholesaler_hq", "wholesaler_branch_name", "wholesaler_branch_number",
}

# Auto-generated fields (not from mapping)
...
def render_field(field: str, mapping_val: str, db_type: str,
                 macro: str | None) -> str:
    """Produce one SELECT expression line (without leading comma)."""
...
def generate_block(row: dict, file_path: str, row_num: int) -> str:
    ...
    # When use_header=TRUE, expand range to include the header row
    ...
    # Build SELECT lines, tracking which fields are Excel-sourced
    lines = []
    excel_sourced = []
    for field in OUTPUT_FIELDS:
        ...
        if field == "transaction_id":
            ...
            mv = row.get(field, "") or ""
            # Track fields actually read from Excel (not NULL, not static, not hardcoded)
    ...
    # Auto blank-row filter: skip rows where all Excel-sourced columns are NULL
    # item_description excluded — validated separately as a required field
...
{select_body}
        ...
        -- Validate types and insert
        INSERT INTO transaction_mapping_credit
        SELECT * FROM {tmp}
        ;
...
CREATE_TABLE_SQL = textwrap.dedent("""\
    -- -----------------------------------------------------------------------
    -- Target table for credit mapping — run this ONCE
    -- -----------------------------------------------------------------------
    CREATE TABLE IF NOT EXISTS transaction_mapping_credit (
...
def main():
    ap = argparse.ArgumentParser(description="Generate DuckDB SQL for transaction_mapping_credit")
    ap.add_argument("--sheet-id",  default=DEFAULT_SHEET_ID,
                    help="Google Sheet ID containing the mapping (reads the 'info' tab)")
    ap.add_argument("--data-dir",  default=DEFAULT_DATA_DIR,
                    help="Directory containing Excel files")
    ap.add_argument("--output",    default=DEFAULT_OUTPUT,
                    help="Output directory for generated SQL files")
    ap.add_argument("--create-table", action="store_true",
                    help="Write 00_create_table_credit.sql")
    args = ap.parse_args()
    ...
    rows = load_mapping(args.sheet_id)
    if not rows:
        print("ERROR: mapping file is empty.", file=sys.stderr)
        sys.exit(1)

    if args.create_table:
        ct = output_dir / "00_create_table_credit.sql"
    ...
    for i, row in enumerate(rows, start=2):
        ...
        if process not in ("TRUE", "1", "YES", "Y"):
            print(f"  SKIP row {i}: process={process!r}")
            continue
